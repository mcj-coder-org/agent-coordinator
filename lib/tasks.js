...
function updateTask(taskFile, newStatus, agentId, maxRetries = 3) {
  const task = JSON.parse(fs.readFileSync(taskFile, 'utf8'));
  task.retries = task.retries || 0;

  // Track retries and auto-reset if under limit
  if (newStatus === 'failed') {
    task.retries += 1;
...
function resetFailedTasks(tasksDir) {
  const tasks = listTasks(tasksDir);
  const reset = [];
  ...
  for (const task of tasks) {
    if (task.status === 'failed') {
      ...
      task.history.push({
        ...
        agent: 'manual-reset',
        timestamp: new Date().toISOString(),
        note: 'manually reset from failed',
      });
      fs.writeFileSync(taskFile, JSON.stringify(task, null, 2) + '\n');
      reset.push(task);
    }
...
if (require.main === module) {
  ...
  if (command === 'claim') {
    ...
    const [taskFile, newStatus, agentId, maxRetries] = args;
    updateTask(taskFile, newStatus, agentId, maxRetries ? parseInt(maxRetries, 10) : 3);
    ...
    const [tasksDir] = args;
    const reset = resetFailedTasks(tasksDir);
    if (reset.length > 0) {
      console.log(`Reset ${reset.length} failed task(s): ${reset.map((t) => t.id).join(', ')}`);
    }
  } else {
